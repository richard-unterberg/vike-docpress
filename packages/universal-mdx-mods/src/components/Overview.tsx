...
import type { UniversalResolvedOverviewItem } from '../runtime/types'
import { useUniversalMdxRuntime } from '../runtime/UniversalMdxProvider'
...
const groupOverviewItems = (items: OverviewItem[]) => {
  ...
  let currentGroup: { dividerText?: ReactNode; items: OverviewLinkItem[] } = { items: [] }
...
const OverviewCard = ({ excerpt, href, title }: OverviewLinkItem) => {
  return (
    <a
      href={href}
      className="group flex h-full flex-col gap-3 rounded-box border border-base-muted-light bg-base-muted-superlight p-5 no-underline transition-colors hover:border-primary-muted-medium hover:bg-base-muted-superlight/50"
    >
